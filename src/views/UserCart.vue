  ...
  setup() {
    ...
    const applyCoupon = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          // 優惠碼套用成功，更新購物車數據
          const cartResponse = await store.getCart() // 重新獲取購物車數據
          ...
          couponMessage.value = response.message || '優惠碼無效或已過期'
          couponError.value = true
          discount.value = 0
          appliedCoupon.value = false
        }
      } catch (error) {
        couponMessage.value = error.response?.data?.message || '優惠碼套用失敗，請稍後再試'
        couponError.value = true
        discount.value = 0
        appliedCoupon.value = false
      } finally {
    ...
    const removeCoupon = async () => {
      ...
      try {
        ...
        couponMessage.value = '移除優惠碼失敗，請稍後再試'
        couponError.value = true
        couponSuccess.value = false
      } finally {
        isApplyingCoupon.value = false
    ...
    onMounted(async () => {
      try {
        ...
        // 檢查購物車是否已經套用了優惠碼
        else if (cartData && cartData.final_total !== undefined && cartData.total !== undefined) {
          // 如果 final_total 小於 total，表示已套用優惠碼
          if (cartData.final_total < cartData.total) {
            ...
            // 如果 store 中沒有優惠碼信息，嘗試從 cartData 中獲取
